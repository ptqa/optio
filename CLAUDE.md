...
- **Repo Task** — `Where` is set. The agent clones the repo into a worktree and opens a PR:
  ...
  Supported git platforms: **GitHub**, **GitLab** (incl. self-hosted via `GITLAB_HOSTS`), **AWS CodeCommit**, and **Bitbucket Cloud**. CodeCommit auths via AWS access keys (or IRSA / instance profile when running on EKS) and uses the AWS CLI credential helper for clones; PR ops go through `@aws-sdk/client-codecommit`. CodeCommit has no native CI or issues — `getCIChecks` returns `[]` (auto-merge still fires on `checksStatus="none"`), `listIssues` returns `[]`, and `reviewTrigger="on_pr"` is recommended over the default `on_ci_pass` for CodeCommit repos.

  Bitbucket **Cloud only** (`bitbucket.org`, API v2.0 — Server/Data Center is not supported). Auths via a repository/workspace/project **access token** stored as the `BITBUCKET_TOKEN` secret: `Authorization: Bearer` for the API, and `x-token-auth` + token for clones (see `scripts/optio-git-credential`). No `gh`/`glab`-style CLI exists in the agent images, so the prompt templates drive PR creation and review submission with `curl` + `jq`. Bitbucket has no PR labels and exposes no mergeability flag, so `labels` is always `[]` and `mergeable` is always `null`; its issue tracker is disabled by default, so `listIssues` maps a 404 to `[]` and `createLabel`/`addLabelsToIssue` throw. Because PR and issue IDs are independent sequences on Bitbucket, `getIssueComments` takes an explicit `resource: "issue" | "pull_request"` discriminator.
